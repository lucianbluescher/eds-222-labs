...
storm_data <- read.csv(here::here('week2/la_storm.csv'))
...
Beta 0 is like the lowest it can go when x is zero. Smallest possible value. Estimated gage height with no precipitation.

Beta 1 is like how much you approximate it going up. For every inch of ranch, the increase in Gage height

sigma how much we could be wrong by, represents spread
...
beta_0_hat <- coef(storm_mod)
y_hat <- 3.9 + 2.2 * x
SST <- sum((storm_data$gage_ht - y_bar)^2)
SSE <- sum((storm_data$gage_ht - y_hat)^2)
# What fraction in the variance in gage height have we explained using precipitation?
R2 <- (SST - SSE) / SST
...
y = sample (Observed Gage heights)

y bar = sample mean (Mean Gage height overall)

y hat = estimated population y (Expected Gage height given preset)
