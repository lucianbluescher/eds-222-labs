...
Difference of proportions of success to failures for Popsicle sticks and garden staples

### State your null and alternative hypotheses

**Q2: What are your null and alternative hypotheses?**

H0: Our null hypothesis is that restoration method has no effect on outcome (No difference)

HA: Our alternative hypothesis is that Popsicle sticks will improve our chance of success in our outcome (Popsicle stick works better than garden staple
...
sample_stat <- restoration |> 
  group_by(treatment) |> 
  summarise(proportion = sum(success)/n())


 test_stat <- sample_stat$proportion[2] - sample_stat$proportion[1]
 test_stat
```
...
perm1000 <- seq(from = 1, to = 1000, by = 1)
...
null_hist <- 
  ggplot(mapping = aes(null_distribution)) +
...
-   THIS ONE —-\> We cannot reject the hypothesis that restoration method does not influence restoration outcome
