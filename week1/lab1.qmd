...
Two numerical things = histogram

Histograms are binning things

One numerical and one categorical thing = bar

Bar plots are not binning things
...
energy <- read_csv('energy-use-per-person.csv') |> 
  clean_names()
...
```{r}
# Sort by Energy consumption per capita
energy_sorted <-  
  arrange(energy, primary_energy_consumption_per_capita_k_wh_person)
```

```{r}
energy_sorted %>% 
  tail(5) |> 
  ggplot(aes(y = primary_energy_consumption_per_capita_k_wh_person, x = entity)) +
  geom_jitter()
...
landfilllmopdata <- clean_names(landfilllmopdata) #|> na.omit(landfilllmopdata)
colnames(landfilllmopdata)
```

```{r}

landfilllmopdata |> 
  filter(ownership_type %in% c("Public", "Private")) |> 
  ggplot(aes(current_landfill_area_acres, percent_methane, color = ownership_type)) +
  geom_point() +
  geom_smooth(method = lm) +
  scale_x_continuous(limits = c(0, 500)) +
  scale_y_continuous(limits = c(30, 60)) +
  scale_color_manual(values = c("Public" = "Red", "Private" = "Blue")) +
   labs(
    title = "Methane % vs. Landfill Area by Ownership",
    x = "Current Landfill Area (acres)",
    y = "Percent Methane",
    color = "Ownership"
  )
